...
export default function Portfolio() {
  ...
  // Parameter 'e' dibuat opsional supaya bisa dipanggil oleh event Swipe
      ...
      <AnimatePresence>
        {selectedProject && (
          ...
          >
            ...
            {/* Tombol Prev (Kiri) - Hanya muncul di Desktop */}
            {selectedProject.images.length > 1 && (
              <button 
                onClick={prevImage}
                className="hidden md:block absolute left-12 text-white/50 hover:text-white transition p-4 bg-white/5 hover:bg-white/10 rounded-full z-50"
              >
            ...
            <div className="relative max-w-6xl w-full h-full flex flex-col items-center justify-center overflow-hidden">
              {/* Indikator Swipe untuk HP */}
              {selectedProject.images.length > 1 && (
                <div className="md:hidden text-white/40 text-[10px] uppercase tracking-widest mb-4 animate-pulse">
                  Swipe gambar ke kiri / kanan
                </div>
              )}

              {/* Gambar dengan fitur Drag/Swipe */}
              <motion.img
                key={currentImgIndex}
                ...
                className={`max-w-full max-h-[65vh] md:max-h-[85vh] object-contain rounded-lg md:rounded-xl shadow-2xl ${selectedProject.images.length > 1 ? 'cursor-grab active:cursor-grabbing' : ''}`}
                onClick={(e) => e.stopPropagation()}
                
                // Konfigurasi Swipe Framer Motion
                drag={selectedProject.images.length > 1 ? "x" : false}
                dragConstraints={{ left: 0, right: 0 }}
                dragElastic={0.2}
                onDragEnd={(e, { offset }) => {
                  if (offset.x < -50) {
                    nextImage(); // Swipe ke kiri = Next
                  } else if (offset.x > 50) {
                    prevImage(); // Swipe ke kanan = Prev
                  }
                }}
              />
              
              <div className="mt-4 md:mt-6 flex flex-col items-center text-white px-4 text-center z-10" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Tombol Next (Kanan) - Hanya muncul di Desktop */}
            {selectedProject.images.length > 1 && (
              <button 
                onClick={nextImage}
                className="hidden md:block absolute right-12 text-white/50 hover:text-white transition p-4 bg-white/5 hover:bg-white/10 rounded-full z-50"
              >
